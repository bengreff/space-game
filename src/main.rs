...
use sunscatter::editor::{screen_to_world, part_at_screen_pos};
use sunscatter::bodies::G;
use sunscatter::game::{Game, GameMode};
use sunscatter::render::{RenderState, PauseAction, OrbitRenderData, ShipRenderData, ShipOrbitData, ShipPartRenderData, OrbitSegmentData, SelectedTarget, StagedPartInfo, TargetPopup, BodyInfoData};
...
fn render_flight_frame(
    ...
    let requests: Vec<sunscatter::render::RenderRequest> =
        render_state.render_requests.drain(..).collect();
    for req in requests {
        use sunscatter::render::RenderRequest;
        match req {
            RenderRequest::TransferNode { position_angle, prograde_dv, radial_dv, time_to_window } => {
                ...
                if let Some(segment) = render_state.current_trajectory.first() {
                    ...
                    let dv = sunscatter::render::ManeuverDeltaV { prograde: prograde_dv, radial_out: radial_dv };
